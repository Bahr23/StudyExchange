        ...
        if user.status == 'admin':
            ...
            if context.args:
                ...
                if order and not chat:
                    ...
                    context.bot.send_message(chat_id=int(order.user_id), text='Чат с исполнителем по поводу заказа ' +
                                                                              order.subject + ' [' + str(order.id)
                                                                              + '] - ' + link)
                    context.bot.send_message(chat_id=int(order.worker_id), text='Чат с заказчиком по поводу заказа '
                                                                                + order.subject + ' [' + str(order.id)
                                                                                + '] - ' + link)

                    pintext = 'Это текст для pin-сообщения\n/admin - вызов админа \n/price - выставить цену \n/done - завершить заказ'
                    pinid = context.bot.send_message(chat_id=update.effective_chat.id, text=pintext, timeout=500)
...
def admin(update, context):
    ...
    if user:
        ...
        for a in admins:
            context.bot.send_message(chat_id=a.id, text=text)
