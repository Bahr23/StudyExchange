...
#
#         media_group = [
#             InputMediaPhoto(media='AgACAgIAAxkBAAIG3l9aBMikfAxqMjVsMPRNLp0P4C3SAAKrrTEb_YXQSmeX8m3i6c5hEmv7lC4AAwEAAwIAA3kAA_tlBAABGwQ', caption='Test1'),
#             InputMediaPhoto(media='AgACAgIAAxkBAAIG3l9aBMikfAxqMjVsMPRNLp0P4C3SAAKrrTEb_YXQSmeX8m3i6c5hEmv7lC4AAwEAAwIAA3kAA_tlBAABGwQ', caption='Test2'),
#         ]
#
#         # media_group = [
#         #     InputMediaDocument(media='BQACAgIAAxkBAAIG5F9aBs6ZdUe-zYkHJGh2-0ThYV8QAAKFCAAC_YXQSnnHk5gTMFfTGwQ'),
#         #     InputMediaDocument(media='BQACAgIAAxkBAAIG5F9aBs6ZdUe-zYkHJGh2-0ThYV8QAAKFCAAC_YXQSnnHk5gTMFfTGwQ')
#         # ]
#
#         # media_group = list()
...
#
#         mid = context.bot.send_media_group(chat_id=MEDIA_ID, media=media_group)
#         print(mid[0])
#         url = 'https://t.me/StudyExchangeMedia/' + str(mid[0].message_id)
#         context.bot.send_message(chat_id=update.effective_chat.id, text='Test \n' + url)
#         # context.bot.forward_message(chat_id=update.effective_chat.id, from_chat_id=MEDIA_ID, message_id=mid[0].message_id, text='text')
#
#
#
    ...
    if user:
        if 'queue' in context.user_data.keys():
            if context.user_data['queue']:
                current_queue(update, context, user)
                return
        text = 'Используйте /profile id_исполнителя'
        if context.args:
            user = User.get(id=int(context.args[0]))
            if user and user.status == 'worker':
                text = get_profile(user.id)
            else:
                text = 'Испольнитель с id ' + context.args[0] + ' не найден'
    ...
    if user:
        if 'queue' in context.user_data.keys():
            if context.user_data['queue']:
                ...
                return
        queue_list = [
            {'subject': 'Выберите предмет:', 'menu': "#subject#0"},
            {'type': 'Выберите тип работы:', 'menu': '#type#0'},
