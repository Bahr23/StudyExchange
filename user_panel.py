        ...
        if user:
            ...
            queue_list = [
                ...
                {'deadline': 'Выберите срок выполнения работы из списка или укажите его в ответном сообщении 👇\n'
                             '\n⚠️ Если требуется помощь в онлайн-режиме, '
            ...
            context.user_data.update({
                ...
                'queue_finish': 'Заказ успешно создан и отправлен на проверку.'
                                ' Вам придёт уведомление, когда он будет одобрен 👌',
...
def faq(update, context):
    ...
    if update.message.chat.id > 0:
        ...
        if user:
            ...
            text = "1. Вы оформляете новый заказ на выполнение студенческой работы через этого бота." \
                   "\n2. Ваша заявка отправляется на рассмотрение менеджеру и спустя время публикуется " \
                   "<a href='https://t.me/StudyExchangeSPbPU'>на канале с заказами.</a>" \
                   "\n3. Исполнители отправляют заявки на выполнение Вашего заказа." \
                   "\n4. После того, как Вы выберете подходящего исполнителя, менеджер создаст общий чат, где Вы" \
                   " должны договориться со специалистом об условиях работы." \
                   "\n5. Вам необходимо внести оплату в полном размере. Эти средства будут удержаны на счёте StudyX, исполнитель не получит к ним доступ до завершения заказа." \
                   "\n6. Когда работа готова, Вы проверяете её и подтверждаете соблюдение оговорённых условий. Теперь исполнитель получит оплату, а Вы сможете оценить его работу." \
                   '\n\nПо всем вопросам пишите Вашему <a href="https://t.me/AlexStudyX">персональному менеджеру</a>.'
